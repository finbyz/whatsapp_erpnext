...
def notify(self, data, label=None):
    ...
    # `data` also carries internal tracking fields (link_to, link_name, contact,
    # document_name, doctype_link_name, message_datetime, date, error_field) that
    # are used later by save_whatsapp_log() to create the WhatsApp Message record.
    # Meta's Graph API only accepts the fields below - sending the extra keys
    # causes a 400 Bad Request. Build a clean payload for the actual API call.
    api_payload = {
    ...
    try:
        frappe.log_error(f"WhatsApp Request Data: {json.dumps(api_payload, indent=2)}", "WhatsApp Message Request")
